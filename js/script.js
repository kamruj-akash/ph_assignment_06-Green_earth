...
const loadCategories = (data) => {
  ...
  data.forEach((btn) => {
    ...
    newBtns.innerHTML = `
        <button
        id="activeBtn_${btn.id}"
        onclick="loadViaCategory(${btn.id})"
...
const loadAllPlants = (data) => {
  ...
  data.forEach((cards) => {
    ...
    newCards.innerHTML = `
        ...
        >
          <img class="w-full" src="${cards.image}" alt="" />
        </div>
        <h3
          onclick="modalDescription(${cards.id})"
          class="font-semibold mt-3 hover:text-[#15803c] hover:underline hover:cursor-pointer"
        >
          ${cards.name}
        </h3>
  ...
  plantsData.forEach((plant) => {
    ...
    newCards.innerHTML = `
        ...
        <div
          class="h-52 bg-gray-200 rounded overflow-hidden md:h-36 flex items-center justify-center"
        >
          <img class="w-full" src="${plant.image}" alt="img" />
        </div>
        <h3
          onclick="modalDescription(${plant.id})"
          class="font-semibold mt-3 hover:text-[#15803c] hover:underline hover:cursor-pointer"
        >
          ${plant.name}
        </h3>
